...
function getComponent(props = { visible: true }) {
    return (React.createElement(FadeInOut, { ...props }));
}
...
describe('FadeInOut', () => {
    ...
    it('should change opacity from 1 to 0', () => {
        ...
        let opacity = wrapper.find('AnimatedComponent')?.prop('style')?.opacity;
        expect(opacity._value).toEqual(1);
        wrapper.setProps({ visible: false });
        jest.runAllTimers();
        opacity = wrapper.find('AnimatedComponent')?.prop('style')?.opacity;
